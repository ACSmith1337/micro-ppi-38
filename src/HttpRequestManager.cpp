...
constexpr int MAX_HTTP_BODY   = 32768; // Cap response size (ADSB.lol returns ~20KB)
...
HttpResult HttpRequestManager::Get(const String& url, const std::vector<std::pair<String, String>>& params, const std::vector<std::pair<String, String>>& headers, int timeout_ms) {
    ...
    // ESP8266 TLS requests must use HTTPClient with an explicit secure client.
    // Raw WiFiClientSecure sockets can buffer the request and never transmit it.
    if (fullUrl.startsWith("https://")) {
        ...
        if (!https.begin(secureClient, fullUrl)) {
            result.errorMessage = "HTTPS begin failed";
            return result;
        }
        for (const auto& header : headers) https.addHeader(header.first, header.second);
        ...
        if (responseCode >= 200 && responseCode < 300) {
            ...
            result.errorMessage = "HTTPS " + String(responseCode);
            result.errorMessage += " (" + https.errorToString(responseCode) + ")";
        }
        https.end();
        return result;
    }
...
HttpStreamResult HttpRequestManager::StreamGet(const String& url, const std::vector<std::pair<String, String>>& params, const std::vector<std::pair<String, String>>& headers) {
    ...
    if (statusCode >= 200 && statusCode < 300) {
        // Wait for body data to arrive before returning client.
        // ArduinoJson stream reader treats read()=-1 as EOF, so if no
        // TCP data has arrived yet, it produces an empty document.
        {
            uint32_t bodyStart = millis();
            while (!client->available() && client->connected()) {
                yield();
                delay(1);
                if (millis() - bodyStart > 5000) break;  // 5s timeout
            }
        }
        result.success = true;
...
HttpResult HttpRequestManager::Post(const String& url, const String& body, const std::vector<std::pair<String, String>>& headers)
{
    ...
    if (!connected && host.equalsIgnoreCase("overpass-api.de")) {
        const char* fallbackIps[] = {"65.109.112.52", "162.55.144.139"};
        for (const char* ip : fallbackIps) {
            client.stop();
            if (client.connect(ip, port)) {
                host = ip;
                connected = true;
                Serial.println("[POST] Overpass backend fallback: " + host);
                break;
            }
        }
    }
    ...
    bool chunked = false;
    int contentLength = ReadHeaders(client, HTTP_TIMEOUT_MS, &chunked);
