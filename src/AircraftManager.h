...
struct SimpleAircraft {
    String icao;
    char icaoBuf[7];  // Fixed buffer for ICAO hex (6 chars + null) — avoids String alloc during JSON parse
    float lat = 0.0f;
    float lon = 0.0f;
    float altitude = 0.0f;
    float heading = 0.0f;
    ...
    char categoryBuf[4];       // Fixed buffer for category (up to 3 chars + null)
    String category = "";     // readsb category (e.g. A1..A7)
    char squawkBuf[5];        // Fixed buffer for squawk (4 chars + null)
    String squawk = "";
};
...
// ─── Airport marker (Y-shaped, lat/lon only — screen coords computed on draw) ──
struct AirportMarker {
    float lat;
    float lon;
    int sx = 999, sy = 999;  // Cached screen coords
    AirportMarker() : lat(0), lon(0) {}
    AirportMarker(float l, float o) : lat(l), lon(o) {}
};
...
public:
    ...
    static volatile bool forceSyncRequested;
...
private:
    ...
    float prevRad = 0.0f;               // Previous radius for airport sync-on-change detection
    uint32_t lastAdsblolFetch = 0;      // 60s rate limit for ADSB.lol
    uint32_t lastAirportFetch = 0;      // 60s rate limit for Overpass
    ...
    static StaticJsonDocument<16384> jsonDoc;
    ...
    void DecayAircraft(uint32_t tickMs);
